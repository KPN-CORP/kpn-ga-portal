...
<div class="space-y-6 text-sm text-gray-800 font-sans max-w-2xl">
    ...
    <div class="bg-white border rounded-xl p-6">
        <form method="POST" action="{{ route('stock-ctl.transaksi.transfer.store') }}">
            @csrf
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-600 mb-1">Area Asal</label>
                <select name="id_area_asal" class="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500" required>
            ...
            <div class="flex justify-end gap-2">
                ...
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg">Transfer</button>
            </div>
        </form>
    </div>
</div>
