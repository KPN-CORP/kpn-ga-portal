...
<div class="space-y-4 sm:space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs sm:text-sm text-gray-600 mt-0.5">
                        Tracking ID: <span class="font-mono font-semibold text-blue-600">{{ $transaksi->no_transaksi }}</span>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-wrap gap-2 mt-2 sm:mt-0">
            ...
            <a href="{{ route('messenger.print', $transaksi->no_transaksi) }}" 
               target="_blank"
               class="inline-flex items-center px-3 py-2 sm:px-4 sm:py-2.5 bg-white border border-gray-300 rounded-lg font-medium text-xs sm:text-sm text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200">
        ...
        <div class="px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-gray-50 to-white border-b border-gray-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
                <div>
                    ...
                    <p class="text-xs sm:text-sm text-gray-600 mt-0.5">
                        Terakhir: {{ \Carbon\Carbon::parse($transaksi->updated_at)->format('d/m/Y H:i') }}
                    </p>
                </div>
                <div class="mt-2 sm:mt-0">
                    ...
                    @endphp
                    <div class="inline-flex items-center px-3 py-1.5 sm:px-4 sm:py-2.5 rounded-lg {{ $config['color'] }} font-semibold text-sm sm:text-base">
                        <i class="{{ $config['icon'] }} mr-2 text-xs sm:text-sm"></i>
                        <span class="hidden sm:inline">{{ $transaksi->status }}</span>
                        <span class="sm:hidden">{{ substr($transaksi->status, 0, 12) }}{{ strlen($transaksi->status) > 12 ? '...' : '' }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="px-4 sm:px-6 py-4 sm:py-6">
            <div class="relative timeline">
                ...
                @if(!empty($timelineData))
                    ...
                    <div class="timeline-item relative {{ $loop->last ? '' : 'pb-4 sm:pb-8' }}">
                        <div class="flex items-start">
                            <div class="flex-shrink-0 w-8 h-8 sm:w-12 sm:h-12 rounded-full {{ $item['color'] }} flex items-center justify-center relative z-10">
                            ...
                            <div class="ml-3 sm:ml-4 flex-1">
                                ...
                                <p class="text-xs sm:text-sm text-gray-500 mt-0.5">
                                    @if(strpos($item['status'], 'Pengiriman Dibuat') !== false)
                ...
                @else
                    ...
                    @if($transaksi->status == 'Terkirim' || $transaksi->status == 'Proses Pengiriman' || $transaksi->status == 'Batal')
                    <div class="timeline-item relative {{ $transaksi->status == 'Terkirim' && $transaksi->gambar_akhir ? 'pb-4 sm:pb-8' : '' }}">
                        <div class="flex items-start">
                            <div class="flex-shrink-0 w-8 h-8 sm:w-12 sm:h-12 rounded-full {{ $transaksi->status == 'Proses Pengiriman' ? 'bg-orange-500' : ($transaksi->status == 'Batal' ? 'bg-red-500' : 'bg-green-500') }} flex items-center justify-center relative z-10">
                    ...
                    @if($transaksi->status == 'Terkirim' && $transaksi->gambar_akhir)
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        <div class="space-y-3 sm:space-y-4">
                            <div>
                                ...
                                <div class="flex items-center p-2 sm:p-3 bg-gray-50 rounded-lg border border-gray-200">
                                    ...
                                    <span class="font-medium text-gray-900 text-sm sm:text-base">{{ ucfirst($transaksi->nama_barang) }}</span>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="p-2 sm:p-3 bg-gray-50 rounded-lg border border-gray-200">
                                    <p class="text-gray-900 text-sm sm:text-base break-words">{{ $transaksi->deskripsi }}</p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="flex items-center p-2 sm:p-3 bg-gray-50 rounded-lg border border-gray-200">
                                    ...
                                    <span class="font-medium text-gray-900 text-sm sm:text-base">{{ \Carbon\Carbon::parse($transaksi->created_at)->format('d/m/Y H:i') }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3 sm:space-y-4">
                            ...
                            <div>
                                ...
                                @if($transaksi->foto_barang)
                                    @if(str_ends_with($transaksi->foto_barang, '.pdf') || str_ends_with($transaksi->foto_barang, '.doc') || str_ends_with($transaksi->foto_barang, '.docx'))
                                    <div class="p-3 sm:p-4 bg-blue-50 rounded-lg border border-blue-200 image-container">
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <p class="font-medium text-gray-900 text-sm sm:text-base truncate" title="{{ $transaksi->foto_barang }}">
                                                    {{ Str::limit($transaksi->foto_barang, 20) }}
                                                </p>
                                                <p class="text-xs sm:text-sm text-gray-500">Dokumen pendukung</p>
                                            </div>
                                        </div>
                                        @if($transaksi->foto_barang_url)
                                            <a href="{{ $transaksi->foto_barang_url }}" 
                                               target="_blank" 
                                    ...
                                    <div class="relative group image-container">
                                        ...
                                        @if($transaksi->foto_barang_url)
                                            <img src="{{ $transaksi->foto_barang_url }}" 
                            ...
                            <!-- Foto Barang Akhir -->
                            @if($transaksi->gambar_akhir)
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-500 mb-1">Bukti Barang Terkirim</label>
                                @if(str_ends_with($transaksi->gambar_akhir, '.pdf') || str_ends_with($transaksi->gambar_akhir, '.doc') || str_ends_with($transaksi->gambar_akhir, '.docx'))
                                <div class="p-3 sm:p-4 bg-green-50 rounded-lg border border-green-200 image-container">
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p class="font-medium text-gray-900 text-sm sm:text-base truncate" title="{{ $transaksi->gambar_akhir }}">
                                                {{ Str::limit($transaksi->gambar_akhir, 20) }}
                                            </p>
                                            <p class="text-xs sm:text-sm text-gray-500">Bukti pengiriman dokumen</p>
                                        </div>
                                    </div>
                                    @if($transaksi->gambar_akhir_url)
                                        <a href="{{ $transaksi->gambar_akhir_url }}" 
                                           target="_blank" 
                                ...
                                <div class="relative group image-container">
                                    ...
                                    @if($transaksi->gambar_akhir_url)
                                        <img src="{{ $transaksi->gambar_akhir_url }}" 
        ...
        <div class="space-y-4 sm:space-y-6">
                    ...
                    <div class="space-y-3 sm:space-y-4">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3 sm:ml-4 min-w-0">
                                ...
                                <p class="font-semibold text-gray-900 text-sm sm:text-base truncate" title="{{ $transaksi->penerima }}">
                                    {{ $transaksi->penerima }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="ml-3 sm:ml-4 min-w-0">
                                ...
                                <p class="font-semibold text-gray-900 text-sm sm:text-base truncate" title="{{ $transaksi->no_hp_penerima }}">
                        ...
                        @if($transaksi->kurir > 0)
                                    ...
                                    <div class="ml-2 sm:ml-4 flex-1 min-w-0">
                                        ...
                                        <div class="mt-1 sm:mt-2">
                                            <div class="pb-2 border-b border-gray-100">
                                                ...
                                                <p class="font-semibold text-gray-900 text-sm sm:text-base truncate" 
                                                title="{{ $kurir->nama_pelanggan }}">
                                                    {{ Str::limit($kurir->nama_pelanggan, 15) }}
                                                </p>
                                            </div>
                                            
                                            @if($kurir->no_hp_pelanggan)
                                            <div class="pt-2">
                                                <p class="text-xs font-medium text-gray-500">No. HP</p>
                                                <p class="font-semibold text-gray-900 text-sm sm:text-base truncate" 
                                                title="{{ $kurir->no_hp_pelanggan }}">
                                                    {{ $kurir->no_hp_pelanggan }}
                                                </p>
                                            </div>
                                            @endif
                                        </div>
                            ...
                            <div class="bg-gray-50 rounded-lg border border-gray-300 p-2 sm:p-4">
                                <div class="flex items-center">
                                    ...
                                    <div class="min-w-0">
                                        <p class="font-medium text-gray-900 text-sm sm:text-base truncate">Kurir ID: {{ $transaksi->kurir }}</p>
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ml-2 sm:ml-4 min-w-0">
                                    ...
                                    <div class="flex items-center mt-0.5 sm:mt-1">
                                        @for($i = 1; $i <= 5; $i++)
                                            @if($i <= $transaksi->penilaian)
                                                <i class="fas fa-star text-yellow-400 text-sm sm:text-lg"></i>
                                            @else
                                                <i class="far fa-star text-gray-300 text-sm sm:text-lg"></i>
                                            @endif
                                        @endfor
                                        <span class="ml-2 sm:ml-3 font-semibold text-gray-900 text-sm sm:text-base">{{ $transaksi->penilaian }}/5</span>
                                    </div>
                                    @if($transaksi->note_penerima)
                                    <div class="mt-1 sm:mt-2 p-2 bg-white rounded border border-gray-200">
                                        <p class="text-xs sm:text-sm text-gray-700 break-words">
                                            ...
                                            "{{ Str::limit($transaksi->note_penerima, 60) }}"
                                        </p>
                                    </div>
                                    @endif
                                </div>
                        ...
                        <div class="bg-yellow-50 rounded-lg border border-yellow-200 p-2 sm:p-4">
                            <div class="flex items-start">
                                ...
                                <div class="ml-2 sm:ml-4 flex-1 min-w-0">
                                    ...
                                    <p class="text-gray-900 text-xs sm:text-sm break-words">{{ $transaksi->note_penerima }}</p>
                                </div>
                            </div>
                        </div>
                        @endif
            ...
            <!-- Quick Actions -->
            @if($transaksi->status == 'Belum Terkirim')
            <div class="bg-gradient-to-br from-red-50 to-white rounded-xl shadow-sm border border-red-100 overflow-hidden">
                ...
                <div class="p-3 sm:p-6">
                    <div class="space-y-2 sm:space-y-3">
                        ...
                        <form action="{{ route('messenger.cancel', $transaksi->no_transaksi) }}" method="POST" id="cancelForm">
                            @csrf
