...
<div class="space-y-6 text-sm text-gray-800 font-sans">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-xl font-semibold text-gray-800">Kartu Tidak Aktif</h2>
            <p class="text-xs text-gray-500">Kartu yang sudah dinonaktifkan oleh admin (tidak tergantung expired)</p>
        </div>
    </div>

    <!-- Filter (selalu terlihat) -->
    <div class="bg-white border rounded-xl p-4">
        <form method="GET" action="{{ route('idcard.inaktif') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
            ...
            <div class="lg:col-span-3 flex flex-col sm:flex-row gap-2 justify-end">
                ...
                <a href="{{ route('idcard.inaktif') }}" class="px-4 py-2 bg-gray-200 rounded-lg text-sm font-semibold text-center">Reset</a>
            </div>
        </form>
    </div>
            ...
            <tbody class="divide-y">
                ...
                <tr>
                    ...
                    <td class="px-4 py-3 flex flex-wrap gap-2">
                        ...
                        @if(isset($canNonaktifkan) && $canNonaktifkan && $item->status == 'approved' && $item->is_active == 0)
                        <button onclick="openAktifModal({{ $item->id }}, '{{ $item->nama }}', '{{ $item->nomor_kartu }}')" class="text-green-600 hover:underline">Aktifkan</button>
                        @endif
                    </td>
                </tr>
                @empty
                <tr><td colspan="6" class="py-10 text-center text-gray-500">Tidak ada kartu tidak aktif</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<!-- Modal Aktifkan -->
<div id="aktifModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 z-50 hidden flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-lg p-6 w-96 max-w-full">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">Aktifkan Kartu</h3>
        <form id="aktifForm" method="POST">
            @csrf
            <p class="text-sm text-gray-600 mb-2">Aktifkan kartu untuk: <span id="aktifNama" class="font-semibold"></span></p>
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1">Nomor Kartu (Opsional)</label>
                <input type="text" name="nomor_kartu" id="aktifNomorKartu" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500" placeholder="Ubah nomor kartu jika perlu">
                <p class="text-xs text-gray-500 mt-1">Kosongkan jika tidak ingin mengubah</p>
            </div>
            <div class="flex justify-end gap-3">
                <button type="button" onclick="closeAktifModal()" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-lg text-sm hover:bg-gray-400">Batal</button>
                <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded-lg text-sm hover:bg-green-700">Aktifkan</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    function openAktifModal(id, nama, nomorKartu) {
        const modal = document.getElementById('aktifModal');
        const form = document.getElementById('aktifForm');
        const namaSpan = document.getElementById('aktifNama');
        const nomorInput = document.getElementById('aktifNomorKartu');

        namaSpan.textContent = nama;
        nomorInput.value = nomorKartu || '';
        form.action = "{{ url('/idcard') }}/" + id + "/aktifkan";
        modal.classList.remove('hidden');
    }

    function closeAktifModal() {
        document.getElementById('aktifModal').classList.add('hidden');
    }

    document.getElementById('aktifModal').addEventListener('click', function(e) {
        if (e.target === this) closeAktifModal();
    });
</script>
@endsection