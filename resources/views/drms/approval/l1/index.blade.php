...
<div class="space-y-6 text-sm text-gray-800 font-sans" x-data="approvalL1Modal()">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Approval Atasan (L1)</h1>
    ...
    <div id="filterSection" class="bg-white border rounded-xl p-4 hidden">
        <form method="GET" action="{{ route('drms.approval.l1.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
            ...
            <div class="lg:col-span-3 flex flex-col sm:flex-row gap-2 justify-end">
                ...
                <a href="{{ route('drms.approval.l1.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-300 text-center">Reset</a>
            </div>
        </form>
    </div>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 space-x-2 whitespace-nowrap">
                            <form action="{{ route('drms.approval.l1.approve', $req->id) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" onclick="return confirm('Setujui permintaan ini?')" class="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-xs font-semibold">Setujui</button>
                            </form>
                            ...
                            <button @click="openDetailModal({{ json_encode($detailData) }})" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-xs font-semibold">Detail</button>
                        </td>
                    </tr>
                    @empty
                        <tr>
                ...
                <tbody>
                    ...
                    <tr class="border-t {{ $loop->first ? '' : 'border-t-2 border-gray-300' }}">
                        ...
                        <td class="px-3 pt-3 pb-1 text-right">
                            ...
                            <form action="{{ route('drms.approval.l1.approve', $req->id) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" onclick="return confirm('Setujui?')" class="bg-green-500 text-white px-2 py-1 rounded text-xs ml-1">Setujui</button>
                            </form>
                            <button @click="openRejectModal({{ $req->id }})" class="bg-red-500 text-white px-2 py-1 rounded text-xs ml-1">Tolak</button>
                        </td>
                    </tr>
                    <tr class="bg-gray-50 border-b">
                        ...
                        <td class="px-3 pb-3 pt-0 text-right">
                            <span class="text-[11px] font-semibold text-yellow-600">Pending L1</span>
                        </td>
                    </tr>
                    @empty
                        <tr>
    ...
    <div x-show="activeTab === 'history'" x-cloak class="bg-white border rounded-xl overflow-hidden shadow-sm">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y">
                    ...
                    @php
                        ...
                        $returnDate = $req->return_date ? \Carbon\Carbon::parse($req->return_date)->format('d/m/Y') . ' ' . $req->end_time : null;
                        $tujuanFull = $req->pickup_location . ' → ' . $req->destination;
                        $statusLabels = ['approved_l1'=>'Disetujui','rejected_l1'=>'Ditolak','approved_admin'=>'Disetujui GA','rejected_admin'=>'Ditolak GA'];
                        $statusColors = ['approved_l1'=>'bg-green-100 text-green-800','rejected_l1'=>'bg-red-100 text-red-800','approved_admin'=>'bg-green-100 text-green-800','rejected_admin'=>'bg-red-100 text-red-800'];
                        $detailData = [
                            'request_no' => $req->request_no,
                            'requester' => ['name' => $req->requester->name],
                            'created_at' => $req->created_at ? \Carbon\Carbon::parse($req->created_at)->format('d/m/Y H:i') : null,
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-10 text-center text-gray-500">Belum ada history.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="px-6 py-3 border-t">
            {{ $historyRequests->appends(array_merge(request()->query(), ['tab' => 'history']))->links() }}
        </div>
        @endif
    </div>
    ...
    return {
        ...
        setTab(tab) {
            ...
            if (tab === 'history') {
                ...
                // switching back to pending isn't paginated, no need to keep page param
