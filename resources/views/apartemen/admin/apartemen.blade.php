...
<div class="p-4 md:p-6">
    ...
    <div class="mb-6 md:mb-8">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3 mb-4 md:mb-6">
            ...
            <div class="w-full lg:w-auto lg:mx-4 lg:flex-1 lg:max-w-md order-first lg:order-none">
                <div class="relative">
                    <form action="{{ route('apartemen.admin.apartemen') }}" method="GET">
            ...
            {{-- Quick Actions - Sama seperti halaman permintaan --}}
            <div class="flex flex-wrap items-center gap-2 lg:gap-3 w-full lg:w-auto">
                ...
                class="inline-flex items-center px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg transition-colors flex-1 lg:flex-none justify-center min-w-[100px] md:min-w-0">
                    ...
                    <span class="font-medium text-blue-700 text-sm truncate">Unit</span> {{-- GANTI text-blue-700 --}}
                    @if($unitCount > 0)
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-3 md:p-4">
            <div class="flex items-center">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-lg md:text-xl font-bold text-gray-900 truncate">{{ $availableUnits }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-3 md:p-4">
            <div class="flex items-center">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-lg md:text-xl font-bold text-gray-900 truncate">{{ $occupiedUnits }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-3 md:p-4">
            <div class="flex items-center">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-lg md:text-xl font-bold text-gray-900 truncate">{{ $maintenanceUnits }}</p>
                </div>
            </div>
        </div>
    </div>

    {{-- Tambah Apartemen Button (Desktop) --}}
    ...
    {{-- Tambah Apartemen Button (Mobile - Floating) --}}
        ...
        <div class="px-3 md:px-4 lg:px-6 py-3 md:py-4 border-b border-gray-200">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <div class="text-xs md:text-sm text-gray-500">
                    Total: <span class="font-medium">{{ $apartemen->total() ?? 0 }}</span> apartemen
                </div>
            </div>
        </div>
        ...
        <div class="p-3 md:p-4 lg:p-6">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="py-3 px-2 md:px-3 lg:px-4">
                                        <div class="min-w-0">
                                            <div class="font-medium text-gray-900 text-sm truncate">{{ $item->nama_apartemen }}</div>
                                            <div class="text-xs text-gray-500 truncate max-w-[150px] md:max-w-none">{{ Str::limit($item->alamat, 40) }}</div>
                                        </div>
                                    </td>

                                    {{-- Penanggung Jawab (Mobile) --}}
                                    <td class="py-3 px-2 md:px-3 lg:px-4 lg:hidden">
                                        <div class="text-xs text-gray-500">PJ:</div>
                                        <div class="text-sm text-gray-900 truncate max-w-[100px]">{{ $item->penanggung_jawab }}</div>
                                    </td>

                                    {{-- Penanggung Jawab (Desktop) --}}
                                    <td class="py-3 px-2 md:px-3 lg:px-4 hidden lg:table-cell">
                                        <div class="text-sm text-gray-900 truncate">{{ $item->penanggung_jawab }}</div>
                                        <div class="text-xs text-gray-500 truncate">{{ $item->kontak_darurat }}</div>
                                    </td>

                                    {{-- Total Unit --}}
                                    <td class="py-3 px-2 md:px-3 lg:px-4">
                                        <div class="flex items-center">
                                            <div class="text-base md:text-lg font-bold text-blue-600 mr-2 md:mr-3">{{ $item->units_count }}</div>
                                            <!-- <div class="hidden md:block">
                                                <div class="text-xs text-gray-500">Tersedia: <span class="font-medium text-green-600">{{ $item->units_ready }}</span></div>
                                                <div class="text-xs text-gray-500">Terisi: <span class="font-medium text-yellow-600">{{ $item->units_terisi }}</span></div>
                                            </div> -->
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-3 px-2 md:px-3 lg:px-4 md:hidden">
                                        <div class="text-xs">
                                            <div class="text-green-600 font-medium">{{ $item->units_ready }} Tersedia</div>
                                            <div class="text-yellow-600 font-medium">{{ $item->units_terisi }} Terisi</div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-3 px-2 md:px-3 lg:px-4 hidden md:table-cell">
                                        <div class="flex items-center space-x-3">
                                            <div class="text-center">
                                                ...
                                                <div class="text-sm font-medium text-green-600">{{ $item->units_ready }}</div>
                                            </div>
                                            <div class="text-center">
                                                <div class="text-xs text-gray-500">Terisi</div>
                                                <div class="text-sm font-medium text-yellow-600">{{ $item->units_terisi }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-3 px-2 md:px-3 lg:px-4">
                                        <div class="flex items-center space-x-2">
                                            <a href="{{ route('apartemen.admin.apartemen.detail', $item->id) }}" 
            ...
            <div class="text-center py-8 md:py-12">
                ...
                <p class="text-gray-500 max-w-xs md:max-w-md mx-auto text-xs md:text-sm">
                    @if(request()->filled('search') || request()->filled('status_unit') || request()->filled('tipe_unit'))
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md transform transition-all">
            <div class="p-4 md:p-6">
                ...
                <form id="addForm" method="POST" action="{{ route('apartemen.admin.apartemen.store') }}">
                    @csrf
                    <div class="space-y-3 md:space-y-4">
                        <div>
                            ...
                            <input type="text" name="nama_apartemen" required
                                   ...
                                   value="{{ old('nama_apartemen') }}">
                            @error('nama_apartemen')
