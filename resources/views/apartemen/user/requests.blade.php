    ...
    <div class="mb-6 md:mb-8">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3 mb-4 md:mb-6">
            ...
            <div class="w-full lg:w-auto lg:mx-4 lg:flex-1 lg:max-w-md order-first lg:order-none">
                <div class="relative">
                    <form method="GET" action="{{ route('apartemen.user.requests') }}" id="searchForm">
            ...
            <div class="flex flex-wrap items-center gap-2 lg:gap-3 w-full lg:w-auto">
                ...
                <button onclick="window.location.href='{{ route('apartemen.user.index') }}'" 
                    ...
                    @if($activeCount > 0)
                    <span class="ml-1 md:ml-2 bg-blue-100 text-blue-800 text-xs px-1.5 md:px-2 py-0.5 rounded-full whitespace-nowrap">{{ $activeCount }}</span>
                    @endif
                </button>
                ...
                <button onclick="window.location.href='{{ route('apartemen.user.requests') }}'" 
                    ...
                    @if($requests->total() > 0)
                    <span class="ml-1 md:ml-2 bg-blue-100 text-blue-800 text-xs px-1.5 md:px-2 py-0.5 rounded-full whitespace-nowrap">{{ $requests->total() }}</span>
                    @endif
                </button>
        ...
        {{-- FILTERS SECTION (Tanpa Filter Jenis) --}}
        <div class="bg-white border border-gray-200 rounded-lg p-4 mb-4">
            <form method="GET" action="{{ route('apartemen.user.requests') }}" class="grid grid-cols-1 md:grid-cols-4 gap-3">
                {{-- Status Filter --}}
                <div>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ $request->tanggal_pengajuan->format('d/m/Y') }}</div>
                            <div class="text-xs text-gray-500">{{ $request->tanggal_pengajuan->format('H:i') }}</div>
                        </td>
                        <!-- STATUS dipindahkan ke urutan kedua -->
                        <td class="px-4 md:px-6 py-4">
                            ...
                            <div class="text-xs text-gray-500 mt-1 max-w-xs">{{ Str::limit($request->reject_reason, 30) }}</div>
                            @endif
                        </td>
                        <!-- JENIS dipindahkan ke urutan ketiga -->
                        <td class="px-4 md:px-6 py-4">
                            <div class="text-sm text-gray-900">{{ $request->jenis_text ?? 'Permintaan Baru' }}</div>
                            <div class="text-xs text-gray-500">{{ $request->penghuni->count() }} penghuni</div>
                        </td>
                        <td class="px-4 md:px-6 py-4">
                            <div class="text-sm text-gray-900">
                                @if($request->assign && $request->assign->unit)
                                    {{ $request->assign->unit->apartemen->nama_apartemen ?? 'N/A' }}
                                @elseif($request->alasan)
                                    {{ Str::limit($request->alasan, 30) }}
                                @else
                                    -
                                @endif
                            </div>
                            @if($request->assign && $request->assign->unit)
                            <div class="text-xs text-gray-500">Unit {{ $request->assign->unit->nomor_unit }}</div>
                            @endif
                        </td>
                        <td class="px-4 md:px-6 py-4">
                            <a href="{{ route('apartemen.user.show', $request->id) }}" 
