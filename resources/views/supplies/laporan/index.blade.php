    ...
    <div class="flex flex-wrap justify-between items-center gap-3">
        ...
        <div class="flex gap-2">
            <form action="{{ route('supplies.laporan.export') }}" method="POST" target="_blank">
                ...
                <button type="submit" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm transition"><i class="fas fa-file-pdf mr-1"></i> PDF</button>
            </form>
            <form action="{{ route('supplies.laporan.export') }}" method="POST" target="_blank">
                @csrf
                <input type="hidden" name="tanggal_awal" value="{{ request('tanggal_awal', $tanggalAwal) }}">
                ...
                <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm transition"><i class="fas fa-file-excel mr-1"></i> Excel</button>
            </form>
        </div>
    </div>
    ...
    <div class="bg-white border rounded-xl p-4">
        <form method="GET" action="{{ route('supplies.laporan.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            <div>
                ...
                <input type="date" name="tanggal_awal" value="{{ request('tanggal_awal', $tanggalAwal) }}" class="w-full border rounded-lg px-3 py-2 text-sm">
            </div>
            <div>
                <label class="text-sm font-medium text-gray-600">Sampai Tanggal</label>
                <input type="date" name="tanggal_akhir" value="{{ request('tanggal_akhir', $tanggalAkhir) }}" class="w-full border rounded-lg px-3 py-2 text-sm">
            </div>
            <div>
                <label class="text-sm font-medium text-gray-600">Bisnis Unit</label>
                <select name="id_bisnis_unit" class="w-full border rounded-lg px-3 py-2 text-sm">
            ...
            <div>
                ...
                <select name="id_barang" class="w-full border rounded-lg px-3 py-2 text-sm">
                    <option value="">Semua</option>
                    @foreach($barang as $b)
