...
<div class="max-w-2xl">
    ...
    <div class="bg-white border rounded-xl p-6">
        <form method="POST" action="{{ route('supplies.stok.masuk.store') }}" id="formStokMasuk">
            @csrf
            ...
            <div class="flex justify-end gap-2"><a href="{{ route('supplies.stok.index') }}" class="px-4 py-2 bg-gray-200 rounded-lg">Batal</a><button type="submit" id="btnSimpanStokMasuk" class="px-4 py-2 bg-blue-600 text-white rounded-lg">Simpan</button></div>
        </form>
    </div>
</div>
<script>
    document.getElementById('formStokMasuk').addEventListener('submit', function(e) {
        var btn = document.getElementById('btnSimpanStokMasuk');
