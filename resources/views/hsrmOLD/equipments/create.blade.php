...
@section('content')
@php
    // PIC (non-admin) yang hanya punya 1 area: Area terisi otomatis & terkunci.
    // Admin, atau user dengan 2+ area, tetap boleh memilih Area.
    $lockedArea = (!$isAdmin && $areas->count() === 1) ? $areas->first() : null;
    $buValue = $lockedArea ? $lockedArea->id_bisnis_unit : old('business_unit_id');
@endphp
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="business_unit_id_display" id="business_unit_id_display" class="w-full border rounded-lg px-3 py-2 bg-gray-100 soft-border" disabled>
                    ...
                    @foreach($businessUnits as $bu)
                        <option value="{{ $bu->id_bisnis_unit }}" {{ $buValue == $bu->id_bisnis_unit ? 'selected' : '' }}>
                            {{ $bu->nama_bisnis_unit }}
                        </option>
                    @endforeach
                </select>
                <input type="hidden" name="business_unit_id" id="business_unit_id" value="{{ $buValue }}">
            ...
            <div>
                ...
                <input type="text"
                       ...
                       name="area_name"
                       list="area-list"
                       value="{{ $lockedArea ? $lockedArea->nama_area : (old('area_name') ?: ($areas->where('id_area_kerja', old('area_id'))->first()->nama_area ?? '')) }}"
                       class="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 soft-border @error('area_id') border-red-500 @enderror @if($lockedArea) bg-gray-100 text-gray-600 cursor-not-allowed @endif"
                       placeholder="Ketik nama area..."
                       autocomplete="off"
                       @if($lockedArea) readonly @endif
                       required>

                <input type="hidden" name="area_id" id="area_id" value="{{ $lockedArea ? $lockedArea->id_area_kerja : old('area_id') }}">
                ...
                @if($lockedArea)
                    <p class="text-xs text-gray-400 mt-1">Terisi otomatis sesuai area Anda.</p>
                @endif
            ...
            <div>
                ...
                <select name="pic_user_id" id="pic_user_id" class="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 soft-border">
                ...
                <p id="pic-hint" class="text-xs text-gray-400 mt-1 hidden">Terisi otomatis dari PIC area ini (bisa diganti).</p>
                @error('pic_user_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            </div>
            @else
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">PIC (Person In Charge)</label>
                <div class="w-full border rounded-lg px-3 py-2 bg-gray-50 text-gray-600 soft-border">
            ...
            {{-- Expired Date --}}
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Expired Date <span class="text-gray-400 font-normal">(opsional)</span></label>
                <input type="date" name="expired_date" value="{{ old('expired_date') }}" class="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 soft-border">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // ===== PIC otomatis mengikuti Area (admin) =====
        // Dipasang SETELAH listener validateArea supaya area_id sudah terisi saat dijalankan.
        const picSelect = document.getElementById('pic_user_id');
        const picHint = document.getElementById('pic-hint');
        const picByArea = @json($picByArea ?? []);
        let lastPicAreaId = areaIdHidden.value || null; // area terakhir yang sudah diproses

        function autoFillPic() {
            if (!picSelect) return;
            const areaId = areaIdHidden.value || null;
            if (areaId === lastPicAreaId) return; // area tidak berubah -> jangan timpa pilihan manual
            lastPicAreaId = areaId;

            const ids = (areaId && picByArea[areaId]) ? picByArea[areaId] : [];
            let chosen = '';
            for (const id of ids) {
                if (Array.from(picSelect.options).some(o => o.value == id)) { chosen = id; break; }
            }
            picSelect.value = chosen;
            if (picHint) picHint.classList.toggle('hidden', !chosen);
        }

        areaInput.addEventListener('input', autoFillPic);
        areaInput.addEventListener('change', autoFillPic);
        areaInput.addEventListener('blur', autoFillPic);
