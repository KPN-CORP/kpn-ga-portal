...
<div x-data="{ showDeleteModal: false }" class="w-full px-2 md:px-4">
    ...
    <div id="printMemoArea" class="bg-white rounded-xl shadow-sm p-6 font-serif">
        <div class="text-right text-sm">{{ $memo->created_at->format('d F Y') }}<br>No. {{ $memo->memo_number }}</div>
        ...
        <p>Mohon disiapkan dana sebesar <strong>Rp {{ number_format($memo->total_amount,0,',','.') }}</strong> untuk {{ $memo->perihal }} dengan rincian:</p>
        ...
        <table class="w-full border mt-2">
            ...
            <tbody>
                ...
                @endforeach
                <tr class="font-bold">
                    <td colspan="{{ 2 + count($dynamicColumns) }}" class="text-right">TOTAL</td>
                    <td class="text-right">Rp {{ number_format($memo->total_amount,0,',','.') }}</td>
                </tr>
            </tbody>
        </table>

        @if($memo->instruksi) <p class="mt-3">{{ $memo->instruksi }}</p> @endif
        ...
        <p class="mt-6">Hormat kami,<br><br>{{ $memo->penandatangan }}<br>{{ $memo->jabatan }}</p>
    </div>
...
<script>
    document.getElementById('printMemoBtn')?.addEventListener('click', () => {
        ...
        printWindow.document.write(`
            <html><head><title>Cetak Memo - {{ $memo->memo_number }}</title>
            <style>
            ...
            </head><body><div class="container">${printContent.innerHTML}</div></body></html>
        `);
