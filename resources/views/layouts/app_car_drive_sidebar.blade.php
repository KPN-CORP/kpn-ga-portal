    ...
    <div class="flex min-h-screen">
        ...
        <aside class="sidebar w-64 bg-white fixed h-full soft-shadow-sidebar overflow-y-auto overflow-x-visible">
            ...
            <div class="p-5 soft-border-bottom border-b flex items-center gap-2">
                <div class="flex items-center space-x-3 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-xs text-gray-500 tracking-wide">Driver</p>
                    </div>
                </div>
            </div>
            ...
            <nav class="p-3 pb-24">
                ...
                <p class="mt-5 mb-1 px-3 text-[11px] font-semibold text-gray-400 uppercase nav-label">Perjalanan</p>
                <ul class="space-y-1">
                    <li>
                        <a href="{{ route('drms.driver.dashboard') }}" title="Jadwal Aktif"
                    ...
                    <li>
                        <a href="{{ route('drms.driver.dashboard') }}?date={{ now()->format('Y-m-d') }}" title="History"
                ...
                <p class="mt-5 mb-1 px-3 text-[11px] font-semibold text-gray-400 uppercase nav-label">Lainnya</p>
                <ul class="space-y-1">
                    {{-- BBM (Driver bisa isi tanpa perjalanan) --}}
                    <li>
                        <a href="{{ route('drms.fuel-logs.create') }}" title="BBM/Charger"
                           class="sidebar-link flex items-center p-3 rounded-lg text-gray-700 {{ request()->routeIs('drms.fuel-logs.create') ? 'active' : '' }}">
                            <i class="fas fa-gas-pump w-5 mr-3 text-gray-400"></i>
                            <span>BBM/Charger</span>
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('drms.fuel-logs.index') }}" title="Riwayat"
                    ...
                    <li>
                        <a href="{{ route('drms.driver.expenses.index') }}" title="Laporan Pengeluaran"
                           class="sidebar-link flex items-center p-3 rounded-lg text-gray-700 {{ request()->routeIs('drms.driver.expenses.*') ? 'active' : '' }}">
                            <i class="fas fa-receipt w-5 mr-3 text-gray-400"></i>
            ...
            <div class="absolute bottom-0 left-0 right-0 p-3 soft-border-top border-t bg-white"
                ...
                <button @click="openProfile = !openProfile" class="w-full flex items-center space-x-3 p-1 rounded-lg hover:bg-gray-50 transition">
                    ...
                    <div class="min-w-0 text-left flex-1">
                        <h3 class="font-medium text-gray-800 truncate">{{ auth()->user()->name ?? 'Driver' }}</h3>
