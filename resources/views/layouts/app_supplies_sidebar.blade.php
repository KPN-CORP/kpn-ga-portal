            ...
            <div class="p-5 soft-border-bottom border-b flex items-center gap-2">
                <div class="flex items-center space-x-3 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-xs text-gray-500 tracking-wide">Supplies</p>
                    </div>
                </div>
            </div>
                ...
                <ul class="space-y-1">
                    ...
                    @php
                        ...
                        if (!empty($suppliesAccess) && ($suppliesAccess['is_admin'] ?? false)) {
                            // Sesuaikan dengan model Supplies Anda
                    ...
                    @if(!empty($suppliesAccess) && ($suppliesAccess['is_user'] || $suppliesAccess['is_admin']))
                        ...
                        <li class="nav-group" :class="{ open: openGroup === 'management' }">
                            ...
                            <div class="dropdown-content" :class="openGroup === 'management' ? 'open' : 'closed'">
                                <ul class="dropdown-child space-y-1 pb-2">
                                    <li>
                                        <a href="{{ route('supplies.barang.index') }}"
                                           class="sidebar-link flex items-center rounded-lg {{ request()->routeIs('supplies.barang.*') ? 'active' : '' }}">
                                            <i class="fas fa-box w-4 mr-3"></i>
                                            <span>Master Barang</span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ route('supplies.stok.index') }}"
                                           class="sidebar-link flex items-center rounded-lg {{ request()->routeIs('supplies.stok.*') ? 'active' : '' }}">
                                            <i class="fas fa-warehouse w-4 mr-3"></i>
                                            <span>Stok</span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ route('supplies.stok.masuk') }}"
                                           class="sidebar-link flex items-center rounded-lg {{ request()->routeIs('supplies.stok.masuk') ? 'active' : '' }}">
                                            <i class="fas fa-arrow-down w-4 mr-3"></i>
                                            <span>Barang Masuk</span>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ route('supplies.laporan.index') }}"
                                           class="sidebar-link flex items-center rounded-lg {{ request()->routeIs('supplies.laporan.*') ? 'active' : '' }}">
                        ...
                        {{-- ===================== REQUEST (DROPDOWN) ===================== --}}
                        <li class="nav-group" :class="{ open: openGroup === 'request' }">
                            <div @click="toggleGroup('request')" title="Request"
                                 class="dropdown-toggle flex items-center justify-between p-3 rounded-lg text-gray-700">
                                <div class="flex items-center">
                            ...
                            <div class="dropdown-content" :class="openGroup === 'request' ? 'open' : 'closed'">
                                <ul class="dropdown-child space-y-1 pb-2">
                                    @if($suppliesAccess['is_user'] || $suppliesAccess['is_admin'])
                                        <li>
                                            <a href="{{ route('supplies.permintaan.index') }}"
                                               class="sidebar-link flex items-center rounded-lg {{ request()->routeIs('supplies.permintaan.index') ? 'active' : '' }}">
                                                <i class="fas fa-list-ul w-4 mr-3"></i>
                                                <span>Permintaan Saya</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{ route('supplies.permintaan.create') }}"
                                               class="sidebar-link flex items-center rounded-lg {{ request()->routeIs('supplies.permintaan.create') ? 'active' : '' }}">
                        ...
                        @if($suppliesAccess['is_admin'])
                        <li>
                            <a href="{{ route('supplies.approval.index') }}" title="Approval"
                               class="sidebar-link flex items-center p-3 rounded-lg text-gray-700 relative {{ request()->routeIs('supplies.approval.*') ? 'active' : '' }}">
                                <i class="fas fa-check-double w-5 mr-3 text-gray-400"></i>
                                <span>Approval</span>
                                @if($pendingApprovalCount > 0)
                                    <span class="badge-count ml-auto text-white text-xs rounded-full px-2 py-0.5">{{ $pendingApprovalCount }}</span>
                                @endif
                            </a>
                        </li>
                        @endif
            ...
            window.sidebarComponent = function() {
                return {
                    ...
                    initSidebar() {
                        ...
                        // Tentukan group mana yang terbuka berdasarkan path
                        const managementPaths = [
                            '/supplies/barang', '/supplies/stok', '/supplies/laporan',
                            '/supplies/stok-masuk'
                        ];
                        const requestPaths = [
                            '/supplies/permintaan'
                        ...
                        } else if (requestPaths.some(p => path.startsWith(p))) {
                            this.openGroup = 'request';
                        }
                    },
