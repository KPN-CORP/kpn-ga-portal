...
<head>
    ...
    <title>GA Portal - Supplies</title>
        ...
        <aside class="sidebar w-64 bg-white fixed h-full soft-shadow-sidebar overflow-y-auto">
            ...
            <div class="p-6 soft-border-bottom flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 opacity-80">Supplies</p>
                </div>
            </div>
            ...
            <nav class="p-4">
                <ul class="space-y-1">
                    {{-- Dashboard GA --}}
                    <li>
                        <a href="{{ route('dashboard') }}" class="sidebar-link flex items-center p-3 rounded-lg text-gray-700">
                    ...
                    @php
                        $suppliesAccess = session('supplies_access', []);
                        $pendingApprovalCount = 0;
                        if (!empty($suppliesAccess) && ($suppliesAccess['is_admin'] ?? false)) {
                            $pendingApprovalCount = \App\Models\Supplies\SuppliesPermintaan::where('status', 'pending')->count();
                        }
                    @endphp

                    @if(!empty($suppliesAccess) && ($suppliesAccess['is_user'] || $suppliesAccess['is_admin']))
                        
                        {{-- ===================== MANAGEMENT DROPDOWN (Admin Only) ===================== --}}
                        @if($suppliesAccess['is_admin'])
                        <li x-data="{ openManagement: false }">
                            <button @click="openManagement = !openManagement" class="sidebar-link flex items-center justify-between w-full p-3 rounded-lg text-gray-700">
                                <span class="flex items-center">
                                    <i class="fas fa-cog mr-3 text-gray-500 opacity-70"></i>
                                    <span>Management</span>
                                </span>
                                <i class="fas fa-chevron-down text-xs transition-transform" :class="{ 'rotate-180': openManagement }"></i>
                            </button>
                            <ul x-show="openManagement" x-collapse class="pl-8 space-y-1 mt-1">
                                <li><a href="{{ route('supplies.barang.index') }}" class="sidebar-link block p-2 rounded-lg text-gray-600 hover:bg-gray-50 {{ request()->routeIs('supplies.barang.*') ? 'active' : '' }}">Master Barang</a></li>
                                <li><a href="{{ route('supplies.stok.index') }}" class="sidebar-link block p-2 rounded-lg text-gray-600 hover:bg-gray-50 {{ request()->routeIs('supplies.stok.*') ? 'active' : '' }}">Stok</a></li>
                                <li><a href="{{ route('supplies.stok.masuk') }}" class="sidebar-link block p-2 rounded-lg text-gray-600 hover:bg-gray-50">Barang Masuk</a></li>
                                <li><a href="{{ route('supplies.laporan.index') }}" class="sidebar-link block p-2 rounded-lg text-gray-600 hover:bg-gray-50 {{ request()->routeIs('supplies.laporan.*') ? 'active' : '' }}">Laporan</a></li>
                            </ul>
                        </li>
                        @endif

                        {{-- ===================== REQUEST DROPDOWN ===================== --}}
                        <li x-data="{ openRequest: false }">
                            <button @click="openRequest = !openRequest" class="sidebar-link flex items-center justify-between w-full p-3 rounded-lg text-gray-700">
                                <span class="flex items-center">
                                    <i class="fas fa-clipboard-list mr-3 text-gray-500 opacity-70"></i>
                                    <span>Request</span>
                                </span>
                                <i class="fas fa-chevron-down text-xs transition-transform" :class="{ 'rotate-180': openRequest }"></i>
                            </button>
                            <ul x-show="openRequest" x-collapse class="pl-8 space-y-1 mt-1">
                                @if($suppliesAccess['is_user'] || $suppliesAccess['is_admin'])
                                    <li><a href="{{ route('supplies.permintaan.index') }}" class="sidebar-link block p-2 rounded-lg text-gray-600 hover:bg-gray-50 {{ request()->routeIs('supplies.permintaan.index') ? 'active' : '' }}">Permintaan Saya</a></li>
                                    <li><a href="{{ route('supplies.permintaan.create') }}" class="sidebar-link block p-2 rounded-lg text-gray-600 hover:bg-gray-50">Buat Permintaan</a></li>
                                @endif
                            </ul>
                        </li>

                        {{-- ===================== APPROVAL (Link langsung) ===================== --}}
                        @if($suppliesAccess['is_admin'])
                        <li>
                            <a href="{{ route('supplies.approval.index') }}" class="sidebar-link flex items-center justify-between p-3 rounded-lg text-gray-700 {{ request()->routeIs('supplies.approval.*') ? 'active' : '' }}">
                                <span class="flex items-center">
                                    <i class="fas fa-check-double mr-3 text-gray-500 opacity-70"></i>
                                    <span>Approval</span>
                                </span>
                                @if($pendingApprovalCount > 0)
                                    <span class="bg-red-500 text-white text-xs rounded-full px-2 py-1 ml-2">{{ $pendingApprovalCount }}</span>
                                @endif
                            </a>
                        </li>
                        @endif
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 opacity-70">
                            @if(!empty($suppliesAccess) && ($suppliesAccess['is_admin'] ?? false))
                                Admin Supplies
                            @elseif(!empty($suppliesAccess) && ($suppliesAccess['is_user'] ?? false))
                                User Supplies
                            @else
                                GA User
                            @endif
