...
class HsrmCertificateController extends Controller
{
    ...
    public function create()
    {
        ...
        // Peta area_id => [user_id PIC] untuk auto-isi dropdown PIC (admin) saat Area dipilih.
        $picByArea = $isAdmin
            ? HsrmUserRole::where('role', 'pic')->orderBy('id')->get()
                ->groupBy('area_id')
                ->map(fn($rows) => $rows->pluck('user_id')->values())
            : collect();
        $certificateTypes = HsrmCertificateType::orderBy('name')->get();
        ...
        return view('hsrm.certificates.create', compact('areas', 'businessUnits', 'pics', 'picByArea', 'isAdmin', 'certificateTypes', 'quotaData'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            ...
            'rekomendasi' => 'nullable|in:recommended',
